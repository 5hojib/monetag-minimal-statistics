...
- **Home-screen widget** — a transparent Android widget showing **Today**, **Yesterday** and a dominant **Total Balance**. Its refresh bolt re-fetches from Monetag **in the background without opening the app** (using your reader-only API key, stored on-device like your other settings); the values roll in with an **odometer animation**, and the totals stay visibly larger than the two daily figures even when you resize the widget small.
